...
int find_Nearest(const vector<double>& pt,
  ...
  // FIRST, COMPUTE LOCATION AND NORMAL AT NEAREST POINT

  // when surface is a sphere 
  // double norm_pt = sqrt(pt[0]*pt[0] + pt[1]*pt[1] + pt[2]*pt[2]);
  // for (int i=0; i<3; i++) nrst.x[i] = pt[i]/norm_pt;
  // nrst.Nrml = nrst.x;
  
  // when surface is NOT a sphere 
  double tol = 1.e-14;
  int n_max = 100;
...
void Generate_Targets(double h,
  ...
  double L = 1.0 + BAND1 * h;
  int N = 2.0*L/h + 1;
  int N_half = L/h + 1;

  bool*** tag = new bool**[N+3];

  for (int i=0; i<N+3; i++) {
    tag[i] = new bool*[N+3];
    for (int j=0; j<N+3; j++) {
      tag[i][j] = new bool[N+3];
    }
  }
  for (int i=0; i<N+3; i++) {
    for (int j=0; j<N+3; j++) {
  ...
  for (int surf_ind=0; surf_ind<N_quad; surf_ind++) {
	...
	for (int k=k0; k <= k0 + 2*width + 3; k++) {
	  ...
	  if ( sqrt(d_sq) <= ((BAND1+1) * h + tol) ) {
	    ...
	    if ( !(tag[i+N_half][j+N_half][k+N_half]) ) {
		...
		// check for pts outside within Band1*h to surface
		if ( ( b >= 0.0 ) && ( abs(b) <= (BAND1 * h + tol) ) ) { 		  
		  temp.x = pt[0];
		  temp.y = pt[1];
		  temp.z = pt[2];
		  temp.nrst_x = pt_surf.x[0];
		  temp.nrst_y = pt_surf.x[1];
		  temp.nrst_z = pt_surf.x[2];
		  for (int ind=0; ind<3; ind++) { 
		    temp.nrst_Nrml[ind] = pt_surf.Nrml[ind];
		    temp.nrst_f[ind] = pt_surf.f[ind];
		  }
		  temp.b = b;
		  Target->push_back(temp); //add the point to the target list

		  tag[i+N_half][j+N_half][k+N_half] = true; // point is in band1
		} // end tagging point
	      }
	      else {
		cout << "ERROR: Finding nearest point on surface failed for a point that needs to be tagged!" << endl;
		cout << "The point is " << pt[0] << "  " << pt[1] << "  " << pt[2] << endl;
	      }
